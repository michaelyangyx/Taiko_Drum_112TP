...
class MenuItem(pygame.font.Font): #mostly similar to menu.py
    def __init__(self,text,font=None,font_size=40,font_color=BLACK,
                 (left,top)=(0, 0)):
        pygame.font.Font.__init__(self,font,font_size)
        self.text=text
...
class Error(object):
    ...
    def __init__(self,screen,function=FUNCTION):
        self.screen=screen
    ...
    def set_items(self):
        items=[]
        items+=["The file you chose is not a *.wav file"]
        return items
    ...
    def run(self):
        ...
        while self.mainloop:
            ...
            self.screen.blit(background,(-80,0))
            item=self.items[-1]
            if self.mouse_mode:
